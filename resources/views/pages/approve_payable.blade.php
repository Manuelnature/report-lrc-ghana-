...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- start page title -->
        <div class="row">
            <center><h4>Payable Approval</h4></center>
        </div>
        <!-- end page title -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="user_dataTable" class="table table-centered dt-responsive nowrap " style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <tbody>
                                    @foreach($all_reviewed_payables as $all_payables)
                                    <tr>
                                        ...
                                        <td>{{ $all_payables->reviewed_by }}</td>
                                        <td class="payable_action_td">
                                            <form method="POST" action="{{ route('reviewed_payable_approve',$all_payables->id) }}">
                                                @csrf
                                            ...
                                            <form method="POST" action="{{ route('reviewed_payable_reject',$all_payables->id) }}">
                                                @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="user_dataTable1" class="table table-centered dt-responsive nowrap " style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <tbody>
                                    @foreach($all_reviewed_cheque_payables as $all_cheque_payables)
                                    <tr>
                                        ...
                                        <td>{{ $all_cheque_payables->reviewed_by }}</td>
                                        <td class="payable_action_td">
                                            <form method="POST" action="{{ route('reviewed_payable_approve',$all_cheque_payables->id) }}">
                                                @csrf
                                                <button type="submit" class="text-success approve_button fw-bold confirm_approve" data-toggle="tooltip" data-placement="top" title="" data-original-title="Approve">Approve</button> 
                                            </form>
                                            <form method="POST" action="{{ route('reviewed_payable_reject',$all_cheque_payables->id) }}">
                                                @csrf
                                                <button type="submit" class="text-danger reject_button fw-bold confirm_reject" data-toggle="tooltip"
                                                data-placement="top" data-original-title="Reject">Reject</button> 
                                            </form>
